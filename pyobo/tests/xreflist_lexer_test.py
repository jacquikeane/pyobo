...
from pyobo.obo_lexer import OboLexerBuilder, XRefListLexer
from pyobo.parsing_exception import OboParsingError
from pyobo.tests.base_test_for_lexer import BaseTestForLexer


class XRefListLexerTest(BaseTestForLexer, unittest.TestCase):
    ...
    def lexing_module(self):
        return XRefListLexer()

    def begin_state(self):
        return OboLexerBuilder.XREF_LIST

    def test_should_recognise_xref_list_start(self):
        actual = self.tokenize("[")
        self.assert_lexing(actual, [["XREF_LIST_START", "[", 1, 0]])

    def test_should_recognise_xref_list_separator(self):
        actual = self.tokenize(",")
        self.assert_lexing(actual, [["XREF_LIST_SEPARATOR", ",", 1, 0]])
 ...
 \t:,\"\\(){}[]@""", 1, 0]])
    ...
    def test_should_recognise_xref_list_end(self):
        actual = self.tokenize("]")
        self.assert_lexing(actual, [["XREF_LIST_END", "]", 1, 0]], pop_states=1)
